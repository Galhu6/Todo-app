...
export const addEvent: RequestHandler = async (req: Request, res: Response, next: NextFunction) => {
    const { accessToken, summary, description, startTime, endTime } = req.body;
    if (!accessToken || !summary || !startTime || !endTime) {
        next(new HttpError(400, "accessToken, summary, start and end time are required"))
        return;
    }
