...
export const editListController: RequestHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  if (
    isNaN(listId) ||
    !userId ||
    (!name && !overallGoal && parentListId === undefined)
  ) {
  ...
  try {
    const editedList = await editList(
      listId,
      userId,
      name,
      overallGoal,
...
export const getListController: RequestHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
  ...
  if (!userId || isNaN(listId)) {
    res
      .status(400)
      .json({ success: false, error: "List and user id are required" });
    return;
  }
...
export const getAllListsController: RequestHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const getDeletedListController: RequestHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
...
export const getSubListsController: RequestHandler = async (
  req: Request,
  res: Response,
  next: NextFunction
) => {
