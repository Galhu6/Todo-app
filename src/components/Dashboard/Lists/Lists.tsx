...
export const Lists = () => {
  ...
  useEffect(() => {
    ...
    Object.values(sublistMap).forEach((arr) =>
      arr.forEach((sublist) => ids.add(sublist.id))
    );
    ids.forEach((id) => {
      allTasks(id)
        .then((tasks) => {
        ...
        .catch((err) =>
          console.error("failed to fetch tasks for progress", err)
        );
    });
  ...
  useEffect(() => {
    const handleClick = (e: MouseEvent) => {
      if (
        ...
        !createListRef.current?.contains(e.target as Node)
      ) {
        setShowCreateList(false);
      }
      if (
        subListParent !== null &&
        subListFormRef.current &&
        !subListFormRef.current.contains(e.target as Node)
      ) {
  ...
  const handleDelete = async (listId: number, parentId?: number) => {
    ...
    if (parentId) {
      setSubListMap((prev) => ({
        ...prev,
        [parentId]: (prev[parentId] || []).filter((l) => l.id !== listId),
      }));
    } else {
  ...
  const handleCreate = async () => {
    ...
    setProgressMap((prev) => ({
      ...prev,
      [created.id]: { completed: 0, total: 0 },
    }));
    setNewListName("");
    setNewListGoal("");
    setShowCreateList(false);
  ...
  const renderListItem = (list: List, parentId?: number): JSX.Element => (
    <li
      ...
      className={`relative group rounded px-2 py-1 transition hover:bg-gray-300 hover:text-gray-800 dark:hover:bg-gray-700/50 dark:hover:text-white ${
        list.id === selectedListId || list.id === secondSelectedListId
          ? "font-bold"
          : ""
      }`}
    >
      ...
      <div className="w-full bg-gray-200 dark:bg-gray-700 h-1 rounded mt-1 overflow-hidden">
        <div
          ...
          style={{
            width: `${
              progressMap[list.id]
                ? progressMap[list.id].total
                  ? Math.round(
                      (progressMap[list.id].completed /
                        progressMap[list.id].total) *
                        100
                    )
                  : 0
      ...
      {subListParent === list.id && (
        <div ref={subListFormRef} className="flex gap-1 mt-1 ml-4">
          ...
          <button
            onClick={() => setSubListParent(null)}
            className="rounded bg-gray-500 px-2 text-white tex-sm"
          >
            Cancel
          </button>
        </div>
      ...
      {editingListId === list.id && (
        <div ref={editFormRef} className="flex gap-1 mt-1">
          <input
            type="text"
            ref={editInputRef}
            value={editName}
  ...
  return (
    <div className="space-y-4 rounded bg-gray-100 dark:bg-gray-800/50 p-4 shadow-lg md:max-w-md w-full mx-auto">
      ...
      {showTrash && (
        <div className="mt-4">
          <h3 className="text-sm text-gray mb-1">Trash</h3>
          <ul className="space-y-1">
            {trash.map((list) => (
              <li
                key={list.id}
                className="rounded bg-gray-200 dark:bg-gray-700 px-2 text-sm dark:text-white"
              >
                {list.name}
              </li>
            ))}
      ...
      {lists.length === 0 && (
        <h3 className="text-center text-gray-400">
          create a new list to add tasks!
        </h3>
      )}
    </div>
  );
