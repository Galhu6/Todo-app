...
export const MicroTasks = ({
  ...
  const toggle = async (id: number) => {
    ...
    if (!current) return;
    const updated = await updateMicroTask(id, {
  ...
  const renderItem = (t: MicroTask): JSX.Element => (
    ...
    >
      <div className="flex items-center gap-1">
        <button
          onClick={() =>
            setExpandedMap((prev) => ({ ...prev, [t.id]: !prev[t.id] }))
          }
        >
          {expandedMap[t.id] ? "▼" : "▶"}
        </button>
        ...
        />
        <span
          className={`flex-grow text-sm ${t.completed ? "line-through" : ""}`}
        >
          {t.description}
        </span>
        <button onClick={() => remove(t.id)} className="text-sx text-red-400">
          X
        </button>
      </div>
      {expandedMap[t.id] && (
        <div className="ml-4 space-y-1">
          <MicroTasks
            ...
            onClose={() =>
              setExpandedMap((prev) => ({ ...prev, [t.id]: false }))
            }
      ...
      <div className="flex gap-1 mt-1 ml-4">
        <input
          ...
          onChange={(e) =>
            setSubInputMap((prev) => ({ ...prev, [t.id]: e.target.value }))
          }
        ...
        <button
          ...
          className="rounded bg-indigo-600 px-2 text-white text-xs"
        >
          Add
        </button>
      </div>
  ...
  return (
    <div
      className="space-y-2 rounded bg-gray-100 dark:bg-gray-800 p-2"
      onDragOver={(e) => e.preventDefault()}
      onDrop={() => handleListDrop()}
    >
      <div className="flex justify-between items-center mb-1">
        <span className="font-semibold text-sm">Subtasks</span>
        <button onClick={onClose} className="text-xs hover:text-red-400">
          X
        </button>
      </div>
      <ul className="space-y-1">{tasks.map(renderItem)}</ul>
      <div className="flex gap-1">
        ...
        <button
          onClick={addTask}
          className="rounded bg-indigo-600 px-2 text-white text-sm hover:bg-indigo-500"
        >
          Add
        </button>
      </div>
